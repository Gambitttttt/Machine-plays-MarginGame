    ...
    if classes == 'random':
        ...
        for id in players.keys():
            ...
            if id == 1:
                ...
                if len(DQN_models) > 0 and len(Q_table_models) > 0:
                    action_type = random.choices(['sber_lover', 'lottery_man', 'manufacturer', 'oil_lover', 'gambler', 'cooperator', 'coop_based', 'memory_based', 'DQN', 'Q_table'], weights=[1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/4, 1/4])[0]
                elif method == 'DQN' and len(DQN_models) > 0:
                ...
                elif method == 'Q_table' and len(Q_table_models) > 0:
                    action_type = random.choices(['sber_lover', 'lottery_man', 'manufacturer', 'oil_lover', 'gambler', 'cooperator', 'coop_based', 'memory_based', 'Q_table'], weights=[1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/16, 1/2])[0] # Пока без DQN
                else:
                    action_type = random.choice(['sber_lover', 'lottery_man', 'manufacturer', 'oil_lover', 'gambler', 'cooperator', 'coop_based', 'memory_based'])
                players[id].action_type = action_type
...
def autonomous_game(n_games, epochs, classes, model, model_name, method, Q_table_models=[], DQN_models=[]):
    ...
    pp.pprint(dict)
    print()
    print(f'Success in 3: {dict[3][2]/(dict[3][2] + dict[3][0.7]+1)}')
    print(f'Success in 4: {(dict[4][3]+dict[4][1.5])/(1+dict[4][3]+dict[4][1.5]+dict[4][1]+dict[4][0.75]+dict[4][0.6]+dict[4][0.5]+dict[4][3/7]+dict[4][3/8])}')
    print(f'Success in 6: {dict[6][3]/(dict[6][3] + dict[6][1]+1)}')
...
def train_with_Q_table(self_play):
    ...
    if not self_play:
        ...
        while True:
            ...
            if n_games % 20000 == 0:
                autonomous_game(n_games=10, epochs=100, classes='assessing_trained', model=Q_table(num_states=11000, num_actions=6), model_name=f'Q_table_without_self_play_{n_games}.npy', method='Q_table',
                    Q_table_models=[],
                    DQN_models=[])
                ...
                plt.plot(dom_learning, label = 'domination', color = 'blue', alpha=0.5)
                plt.legend()
                plt.savefig(f'QTmetrics{n_games}.png')
        ...
        while True:
            ...
            if n_games % 180000 == 0:
                autonomous_game(n_games=10, epochs=100, classes='assessing_trained', model=Q_table(num_states=11000, num_actions=6), model_name=f'Q_table_mid-train_with_self_play_{n_games}.npy', method='Q_table',
                    Q_table_models=names_samples,
                    DQN_models=[])
...
if __name__ == '__main__':                          
    ...
    autonomous_game(n_games=50, epochs=100, classes='assessing_trained', model=DQN(), model_name='DQN_SP_best.pth', method='DQN', # используемые модели должны находиться в директории src
                    Q_table_models=['Q_table_SP_2.npy', 'Q_table_SP_1.npy', 'Q_table_basic.npy'], # названия моделей, руководствующиеся моделью Q_table
                    DQN_models=['DQN_SP_1.pth', 'DQN_basic.pth', 'DQN_SP_best.pth', 'DQN_SP_2.pth'])# названия моделей, руководствующиеся DQN
